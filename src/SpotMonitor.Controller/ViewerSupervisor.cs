...
public sealed class ViewerSupervisor(ViewerTelemetryClient telemetry, ViewerCommandClient commands, RollingFileLogger logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var result = await commands.SendAsync(SpotMonitor.Core.ViewerCommandType.RestartViewer, null, stoppingToken);
            if (result.Success)
            {
            ...
            try
            {
                ...
                Process.Start(new ProcessStartInfo(viewerPath) { UseShellExecute = true });
