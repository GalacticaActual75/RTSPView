...
const compactStyles=document.createElement('link');compactStyles.rel='stylesheet';compactStyles.href='compact.css?v=12.0';document.head.appendChild(compactStyles);
...
function cameraCard(camera,overlay=null,overlayDefinition=null){
 ...
 form.onsubmit=async e=>{e.preventDefault();const state=form.querySelector('.save-state');state.textContent='Saving...';const overlayFields=new Set(['hostCameraSlot','viewportShape','viewportWidthPercent','viewportHeightPercent','viewportHorizontalPositionPercent','viewportVerticalPositionPercent','zoomPercent','imageHorizontalPositionPercent','imageVerticalPositionPercent']),payload={...camera};for(const el of form.elements)if(el.name&&!overlayFields.has(el.name))payload[el.name]=value(form,el.name);try{if(overlayMode){const overlayPayload={camera:payload};for(const name of overlayFields)overlayPayload[name]=value(form,name);overlay=await api(overlayDefinition.endpoint,{method:'PUT',body:JSON.stringify(overlayPayload)});camera=overlay.camera;for(const name of overlayFields){form.elements[name].value=overlay[name];const range=form.querySelector(`input[type="range"][data-sync="${name}"]`);if(range)range.value=overlay[name]}form.dispatchEvent(new Event('input',{bubbles:true}))}else camera=await api(`/api/cameras/${camera.slot}`,{method:'PUT',body:JSON.stringify(payload)});state.textContent='Saved - applying live';setTimeout(()=>refreshThumbnail(thumbnail),3000);setTimeout(()=>state.textContent='',2500)}catch(err){state.textContent=err.message}};
 return form
}
