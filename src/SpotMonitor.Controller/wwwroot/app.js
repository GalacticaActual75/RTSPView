...
async function checkUpdates(){const state=q('#updateState'),button=q('#installUpdate');state.textContent='Checking update channel...';button.hidden=true;try{const update=await api('/api/update');const installed=String(update.installedVersion).replace(/^(\d+\.\d+\.\d+)\.(\d+)$/, '$1-beta.$2');const latest=update.latestVersion?String(update.latestVersion).replace(/^(\d+\.\d+\.\d+)\.(\d+)$/, '$1-beta.$2'):null;state.textContent=update.updateAvailable?`Installed ${installed} - ${latest} is available.`:`Installed ${installed} - ${update.message}`;button.hidden=!update.updateAvailable}catch(err){state.textContent=err.message}}
