...
const compactStyles=document.createElement('link');compactStyles.rel='stylesheet';compactStyles.href='compact.css?v=13.0';document.head.appendChild(compactStyles);
...
function createOverlayPreview(form,overlayDefinition){
 ...
 function viewportPath(shape,x,y,width,height){
  ...
  if(shape===5){
   ...
   if(validCustomPathData(data)&&viewWidth>0&&viewHeight>0){try{path.addPath(new Path2D(data),new DOMMatrix([width/viewWidth,0,0,height/viewHeight,x-viewX*width/viewWidth,y-viewY*height/viewHeight]));return{path,fillRule:'evenodd'}}catch{}}
  }
 ...
 function drawWall(width,height){
  ...
  const viewportClip=viewportPath(viewport.shape,viewport.left,viewport.top,viewport.width,viewport.height),opacity=clamp(number('viewportOpacityPercent',100),20,100)/100;context.save();context.globalAlpha=opacity;context.save();context.clip(viewportClip.path,viewportClip.fillRule);context.fillStyle='#000';context.fillRect(viewport.left,viewport.top,viewport.width,viewport.height);
