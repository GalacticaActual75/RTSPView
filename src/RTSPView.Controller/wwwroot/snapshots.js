...
const snapshotUi = (() => {
  ...
  function init() {
    ...
    panel.onsubmit = async event => {
      ...
      try { const result = await api('/api/snapshots/settings', {method:'PUT',body:JSON.stringify({enabled:panel.elements.enabled.checked,intervalHours:Number(panel.elements.intervalHours.value)})}); apply(result); state.textContent = 'Snapshot settings saved.'; }
  ...
  function apply(settings) { const form = document.querySelector('#snapshotForm'); form.elements.enabled.checked = settings?.enabled ?? false; form.elements.intervalHours.value = settings?.intervalHours ?? 1; }
  ...
  function updated(slot) {
    ...
    for (const image of document.querySelectorAll('.feed-thumbnail,.designer-tile img,.designer-camera img')) {
      if (new URL(image.src,location.href).pathname !== path) continue;
