...
function loadDiagnosticsExclusions(config){
  ...
  if(!section){section=document.createElement('details');section.className='diagnostics-exclusions';const summary=document.createElement('summary');summary.textContent='Diagnostics automatic opening';section.append(summary);const help=document.createElement('p');help.textContent='Select cameras that should not automatically open diagnostics when they have an issue. Their warnings and statistics remain available when you open diagnostics manually.';section.append(help);const list=document.createElement('div');list.className='diagnostics-camera-list';section.append(list);form.querySelector('.actions').before(section);}
