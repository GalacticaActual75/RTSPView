...
async function addOverlay(){
 ...
 try{const overlay=await api('/api/overlays',{method:'POST',body:'{}'}),slot=overlay.camera.slot,id=slot===10?'doorbell':slot===11?'garage':'overlay-'+slot;window.deletedOverlaySlots=(window.deletedOverlaySlots||[]).filter(s=>s!==slot);const form=renderOverlay(overlay,id,slot===10?'/api/doorbell':slot===11?'/api/garage':'/api/overlays/'+slot);adminLayout.selectOverlay(id);const connection=form.elements.name.closest('details');if(connection)connection.open=true;form.elements.name.focus();state.textContent='Overlay added. Enter its name and RTSP URL, choose a display mode, then save.';}
...
function loadDiagnosticsExclusions(config){
  ...
  if(!section){section=document.createElement('details');section.className='diagnostics-exclusions';const summary=document.createElement('summary');summary.textContent='Diagnostics automatic opening';section.append(summary);const help=document.createElement('p');help.textContent='Select streams that should not automatically open diagnostics when they have an issue. Their warnings and statistics remain available when you open diagnostics manually.';section.append(help);const list=document.createElement('div');list.className='diagnostics-camera-list';section.append(list);form.querySelector('.actions').before(section);}
  ...
  for(const camera of cameras){const label=document.createElement('label'),input=document.createElement('input');input.type='checkbox';input.dataset.diagnosticsExclude='';input.name='diagnosticsAutoOpenExcludedSlots';input.value=camera.slot;input.checked=excluded.has(camera.slot);label.append(input,document.createTextNode(' '+camera.name+' · #'+camera.slot));list.append(label);}
}
...
function syncOverlaySource(camera){
 const slot=camera.slot+23;cameraInventory=cameraInventory.filter(c=>c.slot!==slot);const old=grid.querySelector('.camera-card[data-slot="'+slot+'"]');
