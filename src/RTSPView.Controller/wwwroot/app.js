...
combineViewerControls();adminLayout.init();const networkPanel=createNetworkPanel();
...
function showLogin(){clearInterval(window.telemetryTimer);setup.hidden=true;login.hidden=false;app.hidden=true}
function showSetup(){clearInterval(window.telemetryTimer);login.hidden=true;app.hidden=true;setup.hidden=false;setup.appendChild(q('#passwordForm'))}
...
function cameraCard(camera,overlay=null,overlayDefinition=null){
 ...
 form.dataset.slot=camera.slot;form.dataset.kind=overlayMode?overlayDefinition.kind:'camera';form.dataset.label=overlayMode?overlayLabel:`Camera ${mainCameraIds.indexOf(camera.slot)+1}`;form.querySelector('.slot').textContent=overlayMode?`${overlayLabel} overlay`:`Camera ${mainCameraIds.indexOf(camera.slot)+1}`;thumbnail.dataset.slot=camera.slot;
...
function renderOverlay(overlay,id,endpoint){
 ...
 const rename=()=>{const name=form.elements.name.value.trim()||'Unnamed overlay';adminLayout.overlayGrid(id,name);form.dataset.label=name;form.querySelector('.slot').textContent=name;form.elements.enabled.setAttribute('aria-label','Enable '+name);target.querySelector('canvas')?.setAttribute('aria-label','Interactive '+name+' viewport preview')};
...
async function addOverlay(){
 ...
 try{const overlay=await api('/api/overlays',{method:'POST',body:'{}'}),id='overlay-'+overlay.camera.slot;const form=renderOverlay(overlay,id,'/api/overlays/'+overlay.camera.slot);adminLayout.selectOverlay(id);const connection=form.elements.name.closest('details');if(connection)connection.open=true;form.elements.name.focus();state.textContent='Overlay added. Enter its name and RTSP URL, enable it, then save.';}
...
async function load(){const[status,config]=await Promise.all([api('/api/status'),api('/api/config')]);cameraInventory=config.cameras.slice(0,config.cameraCount||9);wallDesigner.load({...config,cameras:cameraInventory});q('#cameraCount').textContent=cameraInventory.length+' cameras';q('#addCamera').disabled=cameraInventory.length>=16;adminLayout.release(status.version);q('#host').textContent=`${status.hostname} - ${status.lanAddresses.join(', ')} - v${status.version}`;grid.replaceChildren(...cameraInventory.map(camera=>cameraCard(camera)));adminLayout.clearExtraOverlays();renderOverlay(config.doorbellOverlay,"doorbell","/api/doorbell");renderOverlay(config.garageOverlay,"garage","/api/garage");for(const overlay of config.additionalOverlays||[])renderOverlay(overlay,"overlay-"+overlay.camera.slot,"/api/overlays/"+overlay.camera.slot);q("#addOverlay").disabled=(config.additionalOverlays||[]).length>=14;const display=q('#displayForm');for(const name of['startFullScreen','preferredMonitor','hideMouseCursor','mouseCursorHideSeconds','showCameraNames','showCameraStats','keepViewerAlwaysOnTop']){const el=display.elements[name],val=config[name];if(el.type==='checkbox')el.checked=val;else el.value=val}q('#clock').textContent=new Date(status.currentTime).toLocaleString();await Promise.all([updateTelemetry(),loadLogs(),checkUpdates(),networkPanel.load()]);clearInterval(window.telemetryTimer);window.telemetryTimer=setInterval(updateTelemetry,2000)}
async function updateTelemetry(){try{const t=await api('/api/telemetry'),s=t.system,v=t.viewer;q('#stats').innerHTML=[['Viewer',t.viewerConnected?'Connected':'Offline'],['CPU',pct(s.cpuPercent)],['RAM',s.ramUsedGb==null?'Unavailable':`${s.ramUsedGb} / ${s.ramTotalGb} GB`],['GPU',pct(s.gpuPercent)],['Video decode',pct(s.gpuVideoDecodePercent)],['Network down',rate(s.networkReceiveMbps)],['Network up',rate(s.networkSendMbps)],['Viewer memory',v?`${v.viewerMemoryMb} MB`:'Unavailable']].map(([l,x])=>`<div class="stat"><b>${x}</b><span>${l}</span></div>`).join('');adminLayout.metrics();for(const card of document.querySelectorAll('.camera-card[data-slot]')){const slot=Number(card.dataset.slot),c=v?.cameras.find(x=>x.slot===slot),box=card.querySelector('.camera-live');if(!c){box.classList.add('offline');box.querySelector('.state').textContent=t.viewerConnected?'Telemetry unavailable':'Viewer offline';box.querySelector('.live-details').textContent=card.dataset.kind!=='camera'?`No live telemetry for ${card.dataset.label} overlay`:`No live telemetry for ${card.dataset.label}`;continue}if(c.streamStartedAt&&card.dataset.streamStartedAt!==c.streamStartedAt){card.dataset.streamStartedAt=c.streamStartedAt;setTimeout(()=>refreshThumbnail(card.querySelector('.feed-thumbnail')),2500)}const live=c.state==='Live'&&!c.frameWarning;box.classList.toggle('offline',!live);box.querySelector('.state').textContent=c.frameWarning?'Stale video':c.state;const resolution=c.width&&c.height?`${c.width}x${c.height}`:'-';box.querySelector('.live-details').textContent=`${c.fps.toFixed(1)} fps - ${resolution} - ${c.codec||'-'} - ${c.bitrateKbps.toFixed(0)} kb/s - ${c.decoder||'Decoder unknown'} - R:${c.reconnectCount}`+(c.frameWarning?` - ${c.frameWarning}`:'')+(c.lastError?` - ${c.lastError}`:'')}}catch{}}
...
async function checkUpdates(){const request=++updateRequest;pendingUpdate=null;q('#updateState').textContent='Checking selected channel...';q('#installUpdate').hidden=true;try{const update=await api('/api/update');if(request===updateRequest)renderUpdate(update)}catch(err){if(request===updateRequest)q('#updateState').textContent=err.message}}
...
q('#updateChannel').onchange=async()=>{
 ...
 pendingUpdate=null;selector.disabled=true;q('#checkUpdates').disabled=true;q('#installUpdate').hidden=true;q('#updateState').textContent='Saving channel selection and checking releases...';
...
q('#installUpdate').onclick=async()=>{
 ...
 const state=q('#updateState');state.textContent='Staging and verifying update...';q('#installUpdate').disabled=true;q('#updateChannel').disabled=true;q('#checkUpdates').disabled=true;
