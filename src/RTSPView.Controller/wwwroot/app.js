...
function cameraCard(camera,overlay=null,overlayDefinition=null){
 ...
 if(overlayMode)overlaySourceUi.bind(form,overlay,cameraInventory); pluginsUi.sourceControls(form); if(pluginsUi.enabled('onvif'))onvifUi.attach(form); adminLayout.card(form,overlayMode);if(overlayMode&&pluginsUi.enabled('automations'))automationUi.decorateOverlay(form);return form
}
...
function layoutStreamInventory(config){
 ...
 return [...main,...(pluginsUi.enabled('pictureInPicture')?raw:[])];
}
