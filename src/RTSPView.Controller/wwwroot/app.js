...
prepareDisplayForm();adminLayout.init();const networkPanel=createNetworkPanel();snapshotUi.init();restartScheduleUi.init();updateNotifications.init();temperatureUi.init();dependencyUi.init();automationUi.init();const connectorPanel=createConnectorPanel();systemTabs.init();
...
function renderOverlay(overlay,id,endpoint){
 ...
 remove.onclick=async()=>{
  ...
  if(!confirm('Delete '+overlay.camera.name+'? Its connection, shape and framing settings will be removed, along with its original stream entry.'))return;
...
async function load(){workspace.reset();const[status,config]=await Promise.all([api('/api/status'),api('/api/config')]);snapshotUi.load(config);loadDiagnosticsExclusions(config);await automationUi.load(config);await restartScheduleUi.load();await temperatureUi.load();cameraInventory=layoutStreamInventory(config);wallDesigner.load({...config,cameras:cameraInventory});q('#cameraCount').textContent=cameraInventory.length+' streams';q('#addCamera').disabled=cameraInventory.filter(c=>!c.overlaySourceSlot).length>=16;adminLayout.release(status.version);adminUi.host(status);grid.replaceChildren(...cameraInventory.map(camera=>camera.overlaySourceSlot?overlaySourceCard(camera):cameraCard(camera)));window.deletedOverlaySlots=config.deletedOverlaySlots||[];adminLayout.clearExtraOverlays();renderOverlay(config.doorbellOverlay,"doorbell","/api/doorbell");renderOverlay(config.garageOverlay,"garage","/api/garage");for(const overlay of config.additionalOverlays||[])renderOverlay(overlay,"overlay-"+overlay.camera.slot,"/api/overlays/"+overlay.camera.slot);adminLayout.selectOverlay(document.querySelector("[data-overlay-target]")?.dataset.overlayTarget);q("#addOverlay").disabled=document.querySelectorAll("[data-overlay-target]").length>=16;const display=q('#displayForm');for(const name of['startFullScreen','preferredMonitor','hideMouseCursor','mouseCursorHideSeconds','showCameraNames','showCameraStats','showTileBorders','keepViewerAlwaysOnTop','showHoverExitButton']){const el=display.elements[name],val=config[name];if(el.type==='checkbox')el.checked=val;else el.value=val}dashboardUX.sync();workspace.load(config);dashboardUX.trackDisplay();q('#clock').textContent=new Date(status.currentTime).toLocaleString();await Promise.all([updateTelemetry(),loadLogs(),checkUpdates(),networkPanel.load(),connectorPanel.refresh()]);clearInterval(window.telemetryTimer);window.telemetryTimer=setInterval(updateTelemetry,2000)}
async function updateTelemetry(){automationUi.refresh();restartScheduleUi.refresh();updateNotifications.refresh();temperatureUi.refresh();dependencyUi.refresh();try{const t=await api('/api/telemetry'),s=t.system,v=t.viewer;const start=q('[data-action="start"]');start.disabled=!!(t.viewerRunning||t.viewerStarting);q('#viewerLifecycleState').textContent=t.viewerPaused?'Viewer intentionally stopped. Automatic recovery is paused.':t.viewerStarting?'Viewer is starting…':t.viewerRunning?'Viewer running. Automatic recovery is enabled.':'Viewer offline. Automatic recovery is enabled.';start.textContent=t.viewerStarting?'Starting viewer…':'Start viewer';start.title=t.viewerRunning?'Viewer is already running':t.viewerPaused?'Resume viewer and automatic recovery':'Start the viewer';q('#stats').innerHTML=[['Viewer',t.viewerConnected?'Connected':'Offline'],['CPU',pct(s.cpuPercent)],['RAM',s.ramUsedGb==null?'Unavailable':`${s.ramUsedGb} / ${s.ramTotalGb} GB`],['GPU',pct(s.gpuPercent)],['Video decode',pct(s.gpuVideoDecodePercent)],['Network down',rate(s.networkReceiveMbps)],['Network up',rate(s.networkSendMbps)],['Viewer memory',v?`${v.viewerMemoryMb} MB`:'Unavailable'],['CPU temperature',temperatureUi.format(s.cpuTemperatureC)],['GPU temperature',temperatureUi.format(s.gpuTemperatureC)]].map(([l,x])=>`<div class="stat"><b>${x}</b><span>${l}</span></div>`).join('');adminLayout.metrics();dashboardUX.health(t);wallDesigner.dimensions(v?.cameras);for(const card of document.querySelectorAll('.camera-card[data-slot]')){const slot=Number(card.dataset.slot),c=v?.cameras.find(x=>x.slot===slot),box=card.querySelector('.camera-live');if(!c){box.classList.add('offline');box.querySelector('.state').textContent=t.viewerConnected?'Telemetry unavailable':'Viewer offline';adminUi.stream(box,null,t.viewerConnected);continue}if(c.snapshotCapturedAt&&card.dataset.snapshotCapturedAt!==c.snapshotCapturedAt){card.dataset.snapshotCapturedAt=c.snapshotCapturedAt;snapshotUi.updated(slot)}if(c.streamStartedAt&&card.dataset.streamStartedAt!==c.streamStartedAt){card.dataset.streamStartedAt=c.streamStartedAt;setTimeout(()=>refreshThumbnail(card.querySelector('.feed-thumbnail')),2500)}box.classList.toggle('offline',c.state!=='Live'||!!c.frameWarning);adminUi.stream(box,c,t.viewerConnected)}workspace.telemetry(t);}catch{q('[data-action="start"]').disabled=true;q('#viewerLifecycleState').textContent='Viewer status unavailable.';workspace.unavailable();for(const stat of document.querySelectorAll('#extraStats .stat'))if(['CPU temperature','GPU temperature'].includes(stat.querySelector('span')?.textContent))stat.querySelector('b').textContent='Unavailable';}}
...
for(const button of document.querySelectorAll('[data-action]'))button.onclick=async()=>{if(button.dataset.action==='start')button.disabled=true;await runControl(`/api/control/viewer/${button.dataset.action}`,{},button.dataset.confirm);await updateTelemetry();};
...
function overlaySourceCard(camera){
 ...
 const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Edit source in Overlays';edit.onclick=()=>{adminLayout.select('overlays');adminLayout.selectOverlay(camera.overlaySourceSlot===10?'doorbell':camera.overlaySourceSlot===11?'garage':'overlay-'+camera.overlaySourceSlot);};form.append(edit);form.querySelector('.overview-configure').onclick=edit.onclick;
