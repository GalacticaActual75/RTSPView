const aircraftUi = (() => {
  ...
  function paint(node,entry){const o=entry.options,actual=snapshots.find(s=>s.key===key(o)),s=actual||(entry.sampleAllowed?sample(o):null),state=freshness(s),all=nearby(o,s);
    ...
    if(state==='unavailable'){node.append(el('div','Aircraft data unavailable','weather-condition'));if(s?.lastError)node.append(el('div',s.lastError,'aircraft-feed-error'));}
      ...
      for(const a of selected){
        ...
        const metrics=el('div',undefined,'aircraft-metrics aircraft-metric');for(const f of o.fields.filter(f=>!['type','owner','airline','destination'].includes(f)))metrics.append(el('div',metric(f,a,o),'weather-highlow'));data.append(metrics);
        if(o.showPhoto&&a.photo&&validPhoto(a.photo)){const figure=el('div',undefined,'aircraft-photo'),image=oldImages.get(a.photo.url)?.shift()||el('img');image.alt=a.photo.representative?'Representative '+a.type+' aircraft':'Aircraft '+(a.registration||a.callsign||a.hex);if(image.getAttribute('src')!==a.photo.url)image.src=a.photo.url;image.referrerPolicy='no-referrer';image.onerror=()=>{figure.remove();body.classList.remove('has-photo');};const credit=el('a','Photo © '+a.photo.photographer+' · '+(a.photo.source||'Planespotters.net')+(a.photo.license?' · '+a.photo.license:''));credit.title=credit.textContent;credit.href=a.photo.link;credit.target='_blank';credit.rel='noopener noreferrer';if(a.photo.representative)figure.append(el('div','Representative photo','aircraft-photo-label'));figure.append(image,credit);body.classList.add('has-photo');body.append(figure);}
        board.append(flight);
      }
    }
  ...
  function editor(initial,onSave,overlay=null,backgroundSlot=null){
    ...
    controls.append(el('p','ADSB.lol receives the search coordinates and radius. Refreshes about every 10 seconds when available, with slower retries after errors. Photos, when enabled, use Planespotters.net by aircraft identifier or registration, with Wikimedia Commons model/airline photos as a labelled fallback. Owner, airline and destination lookups use adsbdb. Destinations are callsign database matches and may differ from the current flight plan.','weather-note'));
