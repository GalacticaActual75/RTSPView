const aircraftUi = (() => {
  const labels = {type:'Aircraft type / registration',altitude:'Altitude',speed:'Ground speed',distance:'Distance / direction',track:'Track',verticalRate:'Climb / descent'};
  const defaults = () => ({location:'',latitude:null,longitude:null,radiusMiles:10,minimumAltitudeFeet:null,maximumAltitudeFeet:null,preset:'featured',units:'imperial',maximumAircraft:5,hideWhenEmpty:true,
  ...
  function sample(o){return {fetchedAt:new Date().toISOString(),aircraft:[['UAL123','B738','N123EX',12400,285],['ASA456','B39M','N456EX',18200,340],['N789EX','C172','N789EX',2200,95]].map((a,i)=>({hex:'sample'+i,callsign:a[0],type:a[1],registration:a[2],altitudeFeet:a[3],speedKnots:a[4],trackDegrees:245,verticalRate:640,latitude:Number(o.latitude||0)+.005*(i+1),longitude:Number(o.longitude||0)+.005*(i+1),positionAt:new Date().toISOString()}))};}
  let snapshots=[],polling=false,fetching=false;const views=new Map();
  function fit(node){if(!node.isConnected)return;const rows=[...node.querySelectorAll('.aircraft-flight')];for(const n of node.querySelectorAll('[data-aircraft-hidden]')){n.hidden=false;delete n.dataset.aircraftHidden;}const footer=node.querySelector('.weather-credit');if(!footer)return;footer.textContent=footer.dataset.label;
    let omitted=false;while(node.scrollHeight>node.clientHeight+1&&rows.length>1){const n=rows.pop();n.hidden=true;n.dataset.aircraftHidden='';omitted=true;}
    if(rows.length){const details=[...rows[0].querySelectorAll('.aircraft-metric')];while(node.scrollHeight>node.clientHeight+1&&details.length){const n=details.pop();n.hidden=true;n.dataset.aircraftHidden='';omitted=true;}}
    if(omitted)footer.textContent='Details hidden · '+footer.dataset.label;node.title=omitted?'Enlarge the aircraft tile or widget to show more details.':'';
  }
  ...
  function paint(node,entry){const o=entry.options,actual=snapshots.find(s=>s.key===key(o)),s=actual||(entry.sampleAllowed?sample(o):null),state=freshness(s),all=nearby(o,s);
    ...
    node.style.visibility=entry.overlay&&o.hideWhenEmpty&&state==='fresh'&&!all.length?'hidden':'visible';
    node.append(el('div',(o.location||'Your location')+' · Nearby aircraft','weather-location'));
    if(state==='unavailable')node.append(el('div','Aircraft data unavailable','weather-condition'));
      ...
      for(const a of selected){const flight=el('div',undefined,'aircraft-flight'),heading=el('div',undefined,'weather-current');heading.append(el('span','✈','weather-icon'),el('strong',a.callsign||a.registration||a.hex.toUpperCase()));flight.append(heading);if(o.fields.includes('type')){const text=metric('type',a,o);if(text)flight.append(el('div',text,'weather-highlow aircraft-metric'));}const metrics=el('div',undefined,'aircraft-metrics');for(const f of o.fields.filter(f=>f!=='type'))metrics.append(el('div',metric(f,a,o),'weather-highlow aircraft-metric'));flight.append(metrics);node.append(flight);}
    }
    const shown=o.preset==='board'?Math.min(all.length,o.maximumAircraft):Math.min(1,all.length);
    const credit=el('a',(entry.sampleAllowed&&!actual?'Sample aircraft · ':'')+(state==='stale'?'Outdated · ':state==='unavailable'?'Unavailable · ':'')+(all.length>shown?`+${all.length-shown} nearby · `:'')+'ADSB.lol · ODbL 1.0','weather-credit');credit.href='https://www.adsb.lol/docs/open-data/api/';credit.target='_blank';credit.rel='noopener noreferrer';credit.dataset.label=credit.textContent;node.append(credit);requestAnimationFrame(()=>fit(node));
  }
  function preview(options,sampleAllowed=false,overlay=false){const node=el('div',undefined,'weather-card aircraft-card'),entry={options:structuredClone(options),sampleAllowed,overlay,featured:null,selectedAt:0};views.set(node,entry);paint(node,entry);resize.observe(node);return node;}
  async function refresh(){polling=true;if(fetching)return;fetching=true;try{snapshots=await api('/api/aircraft');}catch{/* Per-position age continues to expire even on an outage. */}finally{fetching=false;for(const [node,entry] of views){if(!node.isConnected){resize.unobserve(node);views.delete(node);}else paint(node,entry);}}}
  setInterval(()=>{if(polling&&document.visibilityState==='visible')refresh();},5000);
  function overlayBounds(overlay,width,height){const o=overlay.aircraft,margin=Math.min(overlay.margin,width/2,height/2),available=Math.max(0,width-margin*2),availableHeight=Math.max(0,height-margin*2),w=available*overlay.widthPercent/100,size=Math.min(o.fontSize,Math.max(12,(w-o.padding*2)/5)),rows=o.preset==='board'?o.maximumAircraft:1,columns=w-o.padding*2>=400?3:w-o.padding*2>=220?2:1,lines=(o.fields.includes('type')?1:0)+Math.ceil(o.fields.filter(f=>f!=='type').length/columns),h=Math.min(availableHeight,Math.ceil(o.padding*2+42+rows*(Math.max(size*1.7,Math.min(o.iconSize,(w-o.padding*2)*.2))+8+lines*(Math.max(12,size*.6)*1.4+2))));return {width:w,height:h,left:margin+(available-w)*overlay.x/100,top:margin+(availableHeight-h)*overlay.y/100};}
  function editor(initial,onSave,overlay=null,backgroundSlot=null){
    ...
    left.append(el('p','Sample aircraft are labelled. Small cards omit details that do not fit.','weather-note'));
    ...
    button('Search',async()=>{results.replaceChildren();try{const found=await api('/api/weather/search?q='+encodeURIComponent(search.value));for(const place of found.results||[])button([place.name,place.admin1,place.country].filter(Boolean).join(', '),()=>{o.location=place.name;o.latitude=place.latitude;o.longitude=place.longitude;for(const k of ['location','latitude','longitude'])inputs[k].value=o[k];results.replaceChildren();paintPreview();},results);if(!results.children.length)results.append(el('p','No matching locations.'));}catch(e){status.textContent=e.message;}});
    ...
    if(placement)toggle('Hide when no aircraft are nearby','hideWhenEmpty');
    ...
    if(placement){const position=el('details');position.open=true;position.append(el('summary','Position and size'));controls.append(position);const corners=el('div',undefined,'weather-corners');position.append(corners);for(const [label,x,y] of [['Top left',0,0],['Top right',100,0],['Bottom left',0,100],['Bottom right',100,100]])button(label,()=>{placement.x=x;placement.y=y;inputs.x.value=x;inputs.y.value=y;paintPreview();},corners);for(const [label,id,min,max] of [['Maximum width (%)','widthPercent',15,95],['Horizontal position (%)','x',0,100],['Vertical position (%)','y',0,100],['Edge margin','margin',0,80]])input(label,id,'number',position,min,max,false,placement);}
    controls.append(el('p','ADSB.lol receives the search coordinates and radius. Refreshes about every 10 seconds when available, with slower retries after errors. Routes and airline names are not included.','weather-note'));
  ...
  return {defaults,preview,editor,overlayEditor,overlayBounds,refresh,nearby,metric};
})();
