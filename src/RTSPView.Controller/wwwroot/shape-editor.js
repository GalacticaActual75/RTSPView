...
const viewportShapeEditor = (() => {
  ...
  function open(form) {
    ...
    dialog.innerHTML=`<h2 id="shape-editor-title">Viewport shape</h2><p>Choose a shape, or draw where the overlay belongs on the background stream. Use Free draw to drag an outline, or Point outline to click its corners.</p><div class="shape-presets"></div><div class="shape-tools"><button type="button" class="secondary" data-action="draw">Draw outline</button><button type="button" class="secondary" data-action="undo" disabled>Undo</button><label>Smoothing <input type="range" min="0" max="100" value="40" aria-label="Shape smoothing"><output>40%</output></label></div><canvas aria-label="Draw viewport outline over background stream"></canvas><p class="shape-message" role="status"></p><div class="shape-footer"><button type="button" class="secondary" data-action="cancel">Cancel</button><button type="button" data-action="apply" disabled>Use shape</button></div>`;
