...
Recorded local results against beta.3 application code are in opacity-beta3-results.json. The integration test substitutes local media into the real tile using test-only reflection, keeps test snapshots/logs in a temporary folder, and verifies that no native video host is created for the composited tile. All new-compositor checks passed: 20/50/100 percent opacity, ellipse clipping, hide/show, stop/restart with a different color stream, and snapshot capture. Example: the 50 percent red-over-blue pixel was RGB(128,2,127), and 20 percent was RGB(53,3,204).
