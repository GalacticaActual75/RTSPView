...
System → Scheduled restarts supports viewer application restarts or Windows host restarts, disabled by default. Choose an interval of 1–8760 hours, or selected weekdays and a time in the host's time zone. Save explicitly to apply changes. Saving re-arms the schedule; changing a dropdown alone does nothing.
