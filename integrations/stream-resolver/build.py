...
(package / "versions.json").write_text(json.dumps({name: importlib.metadata.version(name)
    for name in ("streamlink", "yt-dlp", "yt-dlp-ejs")} | {"deno": version, "protocol": 1,
    "sourceRevision": hashlib.sha256(b"".join((here / name).read_bytes()
        for name in ("resolver.py", "build.py"))).hexdigest()}), encoding="utf-8")
