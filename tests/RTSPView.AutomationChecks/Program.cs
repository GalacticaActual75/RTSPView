...
var weatherStore = new JsonSettingsStore(Path.Combine(directory,"settings.json"));
var beforeWeather = await weatherStore.LoadAsync();
var commandCount = commands.Count;
await weatherStore.SaveAsync(beforeWeather with {WeatherOverlays=[new() {HostCameraSlot=1,Weather=new() {Location="Fixture",Latitude=40,Longitude=-74}}]});
await Task.Delay(700);
Check(commands.Last().Automation?.Leases.SingleOrDefault()?.Id==initial.Id && commands.Skip(commandCount).All(c=>c.Automation?.Leases.Length>0),"Weather edit reset an active MQTT episode");
cameras = await weatherStore.LoadAsync();
Console.WriteLine("PASS weather-only edits retain active MQTT detection episodes");
