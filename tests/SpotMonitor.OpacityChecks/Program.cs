...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        var folder = Path.Combine(Path.GetTempPath(), "SpotMonitor-OpacityChecks");
        Directory.CreateDirectory(folder);
        var blue = MakeVideo(folder, "blue", 41, 240, 110);
        var red = MakeVideo(folder, "red", 81, 90, 240);
        var app = new Application();
        var main = new Window { Title = "SpotMonitor opacity renderer comparison", Width = 1040, Height = 640, Left = 40, Top = 40, Background = Brushes.Black };
        var grid = new Grid();
    ...
    {
        SpotMonitor.Viewer.OverlayWindowOpacity.Apply(window, (int)Math.Round(alpha * 100d / 255));
    }
    private static async Task VerifyAsync(Window main, LibVLC vlc, string blue)
    {
        ...
        {
            foreach (var overlay in Overlays) SpotMonitor.Viewer.OverlayWindowOpacity.Apply(overlay, percent);
            await Task.Delay(700);
        ...
        SpotMonitor.Viewer.OverlayWindowOpacity.Apply(composited, 50);
        await Task.Delay(500);
        ...
        SpotMonitor.Viewer.OverlayWindowOpacity.Apply(composited, 100);
