...
foreach($project in @('Configuration','Reliability','Connector','ViewerLifecycle','Automation','AutomationHealth','Tapo','LanAccess','RestartSchedule','Maintenance','Weather','Aircraft','LayoutVisibility','Ui','Onvif','StreamingUpdate')) {
...
foreach($check in @('admin-security','connector-http','tapo-http','onvif-http','layout-lan-http','aircraft','wall-layout-presets','wall-proportions','shape-editor','dashboard-ux','viewer-controls','stream-status','application-restart','modern-admin','automation-health','related-choices','preview-refresh','lan-firewall','branding','privacy')) {
    Run-Native 'node' @("tests/$check.checks.cjs",$env:DOTNET_HOST_PATH)
}
foreach($check in @('UpdateHelper','ServiceUpdate','UpdateProgress','ControllerSupervision','InstallationShutdown','Startup')) {
