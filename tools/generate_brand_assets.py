...
source = Image.open(root / "assets" / "branding" / "spotmonitor-icon.png").convert("RGBA")
...
viewer_assets = root / "src" / "SpotMonitor.Viewer" / "Assets"
controller_assets = root / "src" / "SpotMonitor.Controller" / "Assets"
web = root / "src" / "SpotMonitor.Controller" / "wwwroot"
for directory in (viewer_assets, controller_assets, web):
    directory.mkdir(parents=True, exist_ok=True)

ico_sizes = [16, 24, 32, 48, 64, 128, 256]
resized(256).save(viewer_assets / "SpotMonitor.ico", format="ICO", sizes=[(size, size) for size in ico_sizes])
resized(256).save(controller_assets / "SpotMonitor.ico", format="ICO", sizes=[(size, size) for size in ico_sizes])
...
print("Generated SpotMonitor Windows and web icon assets")
