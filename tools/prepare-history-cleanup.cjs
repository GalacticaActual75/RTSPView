...
const sources=['HANDOFF.md','README.md','src/SpotMonitor.Controller/UpdateService.cs','src/SpotMonitor.Core/CameraSettings.cs'];
...
fs.writeFileSync(out+'/mailmap', [...identities].map(old=>'SpotMonitor contributor <maintainer@example.invalid> '+old).join('\n')+'\n');
